...
export class Sade {
  constructor(name, singleCommandMode) {
    ...
    const settings = utils.isObject(singleCommandMode) ? singleCommandMode : {};
  ...
  command(str, desc, opts = {}) {
    ...
    utils.assert(
      utils.existsAsCommandAlias(cmd, this.commandAliases) || cmd in this.tree,
  ...
  option(str, desc, val) {
    ...
    let [flag, alias] = utils.parseOption(str);
  ...
  parse(arr, opts = {}) {
    ...
    if (isSingle) {
      ...
      for (; i < len; i++) {
        ...
        const cmdObj = utils.getCmd(this.tree, tmp);
        const cmdName = cmdObj ? cmdObj.handler.curr : tmp;

        const exists = utils.existsAsCommandAlias(cmdName, this.commandAliases);

        // const tmpTwo = argv._.slice(i, i + 1).join('');
        // const tmpCmdObj = utils.getCmd(this.tree, tmp);
        // const tmpCmd = tmpCmdObj ? tmpCmdObj.handler.curr : tmpTwo;
        if (
          exists ||
          utils.hasOwn(this.tree, cmdName) ||
          utils.hasOwn(this.tree, tmp)
        ) {
          name = tmp;
      ...
      utils.createAliasCommands(this.tree, this.commandAliases);

      cmd = utils.getCmd(this.tree, name);
      // isVoid = cmd;
      if (cmd === null) {
        if (this.settings.defaultCommand) {
          ...
          return utils.printError(bin, `Invalid command: ${tmp}`);
    ...
    if (!isSingle && !cmd) {
      return utils.printError(bin, 'No command specified.');
    }
    ...
    if (!vals || typeof vals === 'string') {
      return utils.printError(bin, vals || 'Parsed unknown option flag(s)!');
    }
    ...
    if (argz.length < reqs.length) {
      ...
      return utils.printError(bin, 'Insufficient arguments!');
    }
  ...
  help(cmdName, log = true) {
    ...
    const str = utils.printHelp(this, cmd);
